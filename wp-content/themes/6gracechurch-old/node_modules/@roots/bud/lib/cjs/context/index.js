...
const makeContext = async (rootDir = process.cwd()) => {
    ...
    const env = new env_1.Env(dir.project);
    const disk = await new disk_1.Disk(dir.project).findConfigs();
    const manifest = await new manifest_1.Manifest(disk).read();
    return new context_1.Context(application.name?.split(`@roots/`).pop() ?? 'bud', dir.cwd, dir.project, manifest, disk, application, env);
};
